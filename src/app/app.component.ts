...
@Component({
  ...
  providers: [ServicioFirmadoXML, ServicioCreacionXML, ServicioUsuario, ServicioCertificado, 
    ...
    ServicioFirmadoXMLNDNC, ServicioEscritorXML, ServicioDecodificador]
})
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //Ejemplo nota de crédtio
    console.log('Nota');
    var data1 = {
      tipoDoc: '01',
      numero: '50605091900310120230300100005010000000016101002595',
      fechaEmision: '2019-09-05T01:41:32-06:00',
      codigo: '01',
      razon: 'Nota de debito'
  }
   this.crearNota(this.XML, 'NC', data1);
  */
  }
  ...
  crearNota(xmlEncoded : string, tipoNota: string, camposNota: object){
    
    this._decoderService.decodificarXML(xmlEncoded).subscribe( 
      
      xml => {
        
        this._writerXML.crearNota(xml.xmlDecoded, tipoNota, camposNota).subscribe(
          result => {
            console.log('wiiiiii ', result);
          },
          error => {
      ...
      error => {
        console.log('Error en decodificador ', error);
      }
    );
